        ...
        for i in np.arange(1, pop_size + 1).reshape(-1):
            if np.random.rand() < p:
                ...
                if np.random.rand() >= 0.5:
                    ...
                    H = np.multiply(Z, X[i, :])
                    GX[i, :] = (np.random.rand() - a) * X[np.random.randint(np.array([1, pop_size])),:] + np.multiply(C, H)
                else:
                    GX[i, :] = X[i, :] - np.multiply(C, (C * (X[i, :] - GX[np.random.randint(np.array([1, pop_size])),:]) + np.random.rand() * (X[i, :] - GX[
